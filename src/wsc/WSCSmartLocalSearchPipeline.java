...
public class WSCSmartLocalSearchPipeline extends BreedingPipeline {
	...
	private boolean swapIsAllowed(Service[] genome, int indexA, int indexB, Set<Service> usedServices, int problemIndex, WSCInitializer init) {
		boolean swap = false;
		// Do not swap services if both of them are unused. Only swap if one is used and the other is not.
		if (usedServices.contains(genome[indexA]) && !usedServices.contains(genome[indexB])) {
			...
			// Only perform swap if used service does not completely dominate the unused one.
			if (!better)
				swap = true;

		}
		else if (usedServices.contains(genome[indexB]) && !usedServices.contains(genome[indexA])) {
			// Do not swap services if the QoS of the unused service is lower than that of the used one
			Service serviceA = genome[indexA];
			Service serviceB = genome[indexB];
	...
	private double normaliseIndividualAvailability(double availability, WSCInitializer init) {
		if (init.maxIndividualAvailability - init.minIndividualAvailability == 0.0)
			...
			return (init.maxIndividualAvailability - availability)/(init.maxIndividualAvailability - init.minIndividualAvailability);
	}
	...
	private double normaliseIndividualReliability(double reliability, WSCInitializer init) {
		if (init.maxIndividualReliability- init.minIndividualReliability == 0.0)
			...
			return (init.maxIndividualReliability - reliability)/(init.maxIndividualReliability - init.minIndividualReliability);
	}
